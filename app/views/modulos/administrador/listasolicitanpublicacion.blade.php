...
<div class="contenedor-interno">
    <div class="centrar-div">
        <h4>Anuncios que solicitan publicación</h4>
    </div>
	    	...
	    	<table class="table table-hover">
		        ...
		        <tbody>
			        ...
			        <tr class="warning">
			            <td>{{ $i++ }}</td>
			            <td>{{ $anuncio->id }}</td>
			            <td>{{ strtoupper(str_limit($anuncio->titulo,30)) }}</td>
			            ...
			            @if(strcmp ($anuncio->estatus_revision,"libre") == 0) 
			            	<td>  <a href="{{ route('admin.revisar', [$anuncio->seccion_title, $anuncio->id]) }}" title="Revisar">Revisar</a>  </td>
			            
			            @elseif(strcmp ($anuncio->estatus_revision,"ocupado") == 0 & ($anuncio->admin==\Auth::id() ) )
			            	<td>  <a href="{{ route('admin.revisar', [$anuncio->seccion_title, $anuncio->id]) }}" title="Revisar">Continua su revision</a>  </td>
			            @else
